...
try {
  ...
  const consumer = [
    ...
    "if (PACKAGE_NAME !== 'contexture' || PACKAGE_VERSION !== '0.16.0rc1' || REFERENCE_SEPARATOR !== '/' || GATEWAY_TOOL_NAMES.join(',') !== [DISCOVER_GATEWAY_NAME, INSPECT_GATEWAY_NAME, OPEN_GATEWAY_NAME, INVOKE_READ_ONLY_GATEWAY_NAME, INVOKE_GATEWAY_NAME].join(',')) throw new Error('missing shared foundation vocabulary');",
    "if (typeof Contexture !== 'function') throw new Error('missing Contexture facade');",
    "if (CoreChannels !== Channels || CoreControllerManager !== ControllerManager || CorePrincipal !== Principal || CoreRootSelection !== RootSelection || typeof coreBindTool !== 'function' || coreCurrentGraph !== currentGraph || coreDefinePostProcess !== definePostProcess) throw new Error('missing SDK-neutral core facade');",
    "const packedProcess = compileRuntimeApplication(defineApplication({ name: 'packed-process', roots: [() => ({ kind: 'role', name: 'owner', description: 'Owner.', instructions: 'Work.', preProcess: () => definePreProcess({ kind: 'role', name: 'prepare', description: 'Prepare.', instructions: 'Prepare.' }), postProcess: () => definePostProcess({ kind: 'role', name: 'finish', description: 'Finish.', instructions: 'Finish.' }) })] })).disclosure.open('owner'); if (packedProcess.pre_process !== 'owner/prepare' || packedProcess.post_process !== 'owner/finish' || !packedProcess.instructions.includes('>>> REQUIRED:') || bindingInstruction('consumer', 'Rule.') !== '===== consumer — binding, follow exactly =====\\nRule.\\n===== end consumer =====') throw new Error('missing packed process API');",
