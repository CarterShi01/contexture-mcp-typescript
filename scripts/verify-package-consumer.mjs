...
try {
  ...
  const consumer = [
    ...
    "import { Channels, CompileLevel, Contexture, ContextureError, ControllerManager, DeclarationError, DISCOVER_GATEWAY_NAME, GATEWAY_TOOL_NAMES, InMemoryTelemetry, INVOKE_GATEWAY_NAME, INVOKE_READ_ONLY_GATEWAY_NAME, LookupFailure, NodeNotFoundError, OPEN_GATEWAY_NAME, OutsideSelectionError, PACKAGE_NAME, PACKAGE_VERSION, Principal, REFERENCE_SEPARATOR, RootOutsideSelectionError, RootSelection, SelectedGraph, SurfaceSelection, WrongDoorError, branchesOf, compileNode, currentGraph, currentPrincipal, defineApplication, defineTool, groupCards, membersOf, reportTelemetry, withGraph } from '@contexture/mcp';",
    "import { trace } from '@contexture/mcp/inspection';",
    "import { newProject } from '@contexture/mcp/cli';",
    "import { Auth, buildServer, claudeCodeConfig, compileRuntimeApplication, ContextureOptions, DisclosureAPI, DISCLOSURE_GATEWAY, EXECUTION_GATEWAY, ExecutionAPI, Gateway, GATEWAY, HeaderRootSelector, HeaderSurfaceSelector, Launch, ROOTS_HEADER, SELECT_HEADER, ServeError, unresolvedMessage } from '@contexture/mcp/server';",
