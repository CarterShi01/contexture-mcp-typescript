...
try {
  ...
  const consumer = [
    ...
    "if (typeof DisclosureAPI !== 'function') throw new Error('missing DisclosureAPI facade'); const packedNavigation = new DisclosureAPI(rawRuntime.disclosure); if (packedNavigation.tools.length !== 2 || (await packedNavigation.discover()).skills.length !== 1) throw new Error('missing packed DisclosureAPI navigation surface');",
