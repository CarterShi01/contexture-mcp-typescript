...
try {
  ...
  const consumer = [
    ...
    "import { availableTemplates, CLI_VERSION, main as cliMain, newProject, UsageError } from '@contexture/mcp/cli';",
    "import { ApplicationRuntime, Auth, buildInstructions, buildServer, claudeCodeConfig, cliCommands, codexConfig, COMMAND_CLOSING, commandDescription, compileRuntimeApplication, compileStructuralApplication, ContextureOptions, currentTelemetry as serverCurrentTelemetry, cursorConfig, DisclosureAPI, DISCLOSURE_GATEWAY, EXECUTION_GATEWAY, ExecutionAPI, Gateway, GATEWAY, GATEWAY_TOOLS, GOTO_DESCRIPTION, HeaderRootSelector, HeaderSurfaceSelector, InMemoryTelemetry as ServerInMemoryTelemetry, INSTRUCTIONS_LIMIT, Launch, neutralInstructions, PREAMBLE, Refused, REF_RULE, reportTelemetry as serverReportTelemetry, ROOTS_HEADER, ROSTER_BUDGET, SELECT_HEADER, SELF_CONTAINED_PREFIX, ServeError, signpost, SystemAPI, truncatedCompletion, unresolvedMessage, withTelemetry as serverWithTelemetry } from '@contexture/mcp/server';",
